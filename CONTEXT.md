...
WPDevPilot is a macOS menu-bar app that runs local WordPress `.test` sites on top of
Homebrew services. This glossary defines the domain language used across the app.

## Language

**Site**:
A single local WordPress `.test` installation WPDevPilot manages, backed by a webroot
directory on disk and a database.
_Avoid_: Project, install, instance

**Agent**:
An AI-provider CLI tool (e.g. Claude Code, Codex) that WPDevPilot can launch in a
...
terminal rooted at that Site's directory. WPDevPilot curates and launches Agents
...
hiding to the tray and is torn down when its shell exits or WPDevPilot quits.
