# WPXen — Feature Roadmap

A comparison of WPXen against leading local WordPress/PHP development tools, and a
...
| Capability                            | Local | Herd        | Studio                | **WPXen today** |
| ------------------------------------- | ----- | ----------- | --------------------- | --------------- |
...
## Current WPXen features (baseline)

The **Also in** tag names which competitors ship a comparable feature — useful for
spotting where WPXen is already at parity and where it stands alone.
...
- One-click admin "magic login" — _WPXen only_
...
- Export / import site: portable files+SQL zip; imports WPXen, generic, and `.wpress` archives — _Also in: Local, Studio_
...
- Agent Launcher terminal: per-site AI-agent sessions in an embedded terminal with native clipboard/keys (Cmd+C/V, Shift+Enter multiline, line-edit chords), find (Cmd+F), clear (Cmd+K), WebGL rendering, Cmd+click file/URL links, drag-drop paths, OSC tab titles, and a session cache that survives tab switches — _WPXen only (agentic coding: also in Studio Code)_
...
> reference link to study how the competitor solves it, then implement the WPXen
...
4. **Export / Import site** — ✅ **Shipped.** Exports files + SQL into a portable
   zip; imports WPXen archives, generic files+SQL zips, and `.wpress`
