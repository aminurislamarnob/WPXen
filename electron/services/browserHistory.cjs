...
//
// Superset keeps this in SQLite; WPHerd has no SQL layer, so it lives in the
...
function searchEntries(entries, query, limit = 8) {
  ...
  for (const entry of entries) {
    ...
    // Compare against the URL without its scheme too, so "wpherd" matches
    // "https://wpherd.test" as a prefix rather than a mid-string hit.
