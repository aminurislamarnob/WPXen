...
function createResolverFile() {
  const resolverContent = `# WPXen DNS resolver for .${TLD} domains\nnameserver 127.0.0.1\n`;
  ...
  const tmpFile = path.join(os.tmpdir(), `wpxen-resolver-${TLD}`);
  ...
  const reason =
    `WPXen wants to configure macOS so that .${TLD} sites resolve to your ` +
    'local machine.';
  try {
