...
// keeps macOS "App Background Activity" down to just WPDevPilot.
//
...
function getBaseDir() {
  ...
  try {
    ...
    baseDir = path.join(require('os').tmpdir(), 'wpdevpilot-procman');
  }
  return baseDir;
}
...
async function start(spec) {
  ...
  if (typeof spec.conflictProbe === 'function') {
    ...
    if (conflicted) {
      ...
      setState(entry, name, 'stopped');
      throw new Error(
        `${name} is already running outside WPDevPilot. Stop the other instance (e.g. \`brew services stop ${name}\`) and try again.`
      );
    }
  }
...
function signal(name, sig) {
  ...
  if (!entry || !entry.child) {
    throw new Error(`${name} is not running under WPDevPilot.`);
  }
