...
Implementation spec for bringing WPHerd's Agents-screen file explorer and git
...
- Persist mode in `localStorage` key `wpherd.changesViewMode`
...
file. Superset renders diffs in a pane; WPHerd renders them as a special
...
- Changes row click → open diff tab. Row context menu (new, reuse the
  ...
  **Open File**, **Copy Path**, **Copy Relative Path**, **Reveal in
  Finder**. (No "Open in Editor"/external — WPHerd has no external-editor
  concept.)
...
plain `git`/`touch` in the site dir — never in the WPHerd repo itself).
