# Superset settings → WPDevPilot global settings

A read of Superset's settings surface (`superset-sh/superset`,
`apps/desktop/src/renderer/routes/_authenticated/settings/**`) and what of it WPDevPilot
should adopt as **global settings**.

WPDevPilot's settings today live in one scrolling page,
...
## 2. Superset's actual setting inventory (the parts with a WPDevPilot analogue)
...
1. **Split Settings into routed sections with a settings sidebar.** Suggested
   ...
   - _System_ — Services · DNS & HTTPS · Permissions · Sharing · Experimental ·
     Dependencies · About
2. **Kill the Save button.** Persist per row on change (WPDevPilot's `JsonStore`
...
3. **Build the setting registry** — `{ id, section, title, description, keywords }`
   — and a search box that filters sections + rows. WPDevPilot has enough settings to
   justify it the moment tier 1 lands.
4. **Adopt the global-default → per-site-override pattern explicitly.** WPDevPilot
...
### Tier 1 — settings WPDevPilot is missing and clearly needs
...
| **External editor / browser / terminal app** | External Tools | Superset's Links section. WPDevPilot calls `shell.openPath` today, so "Open in editor" is whatever macOS decided. Offer VS Code / Cursor / PhpStorm / Sublime / custom command, plus which browser opens a site.     |
...
- **Appearance section — colour scheme.** WPDevPilot deliberately follows the macOS
...
ui: UIColors, terminal?: TerminalColors, editor?: EditorThemeOverrides }` —
  ...
  - `ui` — the full chrome token set (background, card, popover, primary,
    ...
    vocabulary as WPDevPilot's `src/index.css` variables.
  ...
  - `editor` — CodeMirror/diff colours (gutter, active line, selection, search,
    ...
    tokens** — exactly what WPDevPilot's `src/lib/editorTheme.js` already does by
  ...
  For WPDevPilot this is mostly plumbing: `src/lib/theme.js` already holds
...
- **Typography — terminal and code editor fonts.** Superset stores two independent
  ...
  WPDevPilot hardcodes a single `MONO_STACK` for both the xterm terminal and the
...
- **Terminal presets.** Superset's preset = name + description + cwd + commands.
  WPDevPilot's version writes itself: per-site presets like `wp db cli`,
...
- **macOS Permissions page.** Superset's status + deep-link pattern. WPDevPilot's
...
- **Models.** Only meaningful once WPDevPilot itself calls a model (AI site naming,
...
- **Account / Organization / Teams / Billing / Hosts / Integrations.** Superset is
  a multi-user cloud product; WPDevPilot is single-user and local. Skip.
- **Git & Worktrees.** Only if WPDevPilot grows a git workflow beyond `git.cjs`.

### Explicitly not worth copying

- Per-setting `v1` / `v2` variant gating — that exists to run two UIs at once.
- Markdown render style, file open mode (split pane / new tab) — no WPDevPilot surface
  needs them yet.
- The in-app browser toggle: WPDevPilot opens sites in a real browser on purpose.
