...
checkout at `reference/superset-main`) against WPDevPilot's Agents-screen
...
## WPDevPilot terminal today (`src/components/Terminal.jsx`)
...
| C7  | **Scrollback 5000** (`DEFAULT_TERMINAL_SCROLLBACK`), native scrollbar hidden (fit addon reserves its width otherwise), custom wheel handler                    | `config.ts`, `shared/constants.ts`                       | WPDevPilot is on xterm's default 1000 lines.             |
...
| C9  | **Perf plumbing**: write coalescer + parser-idle gate (fit/resize only when the parser is idle)                                                                | `lib/terminal/write-coalescer.ts`, `parser-idle-gate.ts` | Only worth porting if WPDevPilot sees jank.              |
| C10 | Flash-free theming: initial terminal theme read synchronously before store hydration; theme object swapped live on change                                      | `helpers.ts` `getDefaultTerminalTheme`                   | WPDevPilot already swaps theme live via `onThemeChange`. |
...
| #   | Feature                                                                                                                                                                                                                                       | Superset source                                                | Notes                                                                                                                            |
| --- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| E1  | **xterm instance cache across tab switches** — the terminal opens into a detached wrapper div that's re-parented on mount, so switching tabs never disposes xterm or drops scroll position/images; the data stream keeps writing while hidden | `helpers.ts` `createTerminalInWrapper`, `v1-terminal-cache.ts` | WPDevPilot re-creates xterm per mount and replays the ring buffer — loses scroll position, selections, images; visible re-paint. |
| E2  | **Reconnect with backoff** (max 5 tries) + "[Connection lost. Reconnecting…]" line + connection indicator                                                                                                                                     | `Terminal.tsx`                                                 | Less relevant: WPDevPilot's pty is in-process.                                                                                   |
| E3  | **Session-killed overlay with Restart**; typing into an exited terminal restarts it                                                                                                                                                           | `components/SessionKilledOverlay`, `useTerminalLifecycle`      | WPDevPilot's exit overlay only offers Close.                                                                                     |
| E4  | **Close-confirm** when a terminal has a running process, with persisted "don't ask again"                                                                                                                                                     | `stores/terminal-close-confirm/`                               | WPDevPilot confirms only at app quit (ADR 0001).                                                                                 |
| E5  | **Tab titles**: OSC title changes + typed-command heuristic (`commandBuffer` — the echoed command becomes the tab name); **CWD tracking** via OSC 7 so new terminals inherit the cwd                                                          | `useTerminalLifecycle`, `commandBuffer.ts`, `parseCwd.ts`      | WPDevPilot tabs are static agent names.                                                                                          |
...
| E7  | Cold restore of a previous daemon session (scrollback snapshot + "restored" overlay), background-terminal reattach dropdown                                                                                                                   | `useTerminalColdRestore`, `TerminalSessionDropdown`            | **Rejected for WPDevPilot** by ADR 0001 (no daemon).                                                                             |
...
| #   | Feature                                                                                                                                                                                                   | Superset source                             | Notes                                                   |
| --- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------- | ------------------------------------------------------- |
| F1  | **Drag & drop into the terminal**: Finder file drops and internal file-tree drags insert shell-escaped path(s) (`shell-quote`)                                                                            | `Terminal.tsx` `handleDrop`                 | Pairs naturally with WPDevPilot's Files tree.           |
...
| F4  | Focus-follows-terminal (textarea focus listener drives the app's focused-pane state)                                                                                                                      | `helpers.ts` `setupFocusListener`           | WPDevPilot single-pane; matters only if panes multiply. |

---

## Recommendation — what WPDevPilot should implement
...
1. **Custom key handler (A1 + A2)**: port `clipboard-shortcuts.ts` +
   ...
   Cmd+A → `selectAll()`. WPDevPilot is macOS-only, so the Win/Linux branches
...
13. **File-path links (D1)**: port the local-link detector with a
    ...
    `files.assertInRoot`) and open matches in WPDevPilot's CodeEditor at
...
15. **Tab title & cwd (E5)**: `onTitleChange` → session tab label;
    OSC 7 cwd parse is optional (WPDevPilot sessions are site-rooted anyway).
...
- **Daemon cold restore / session dropdown / reconnect transport (E2, E7)** —
  contradicts ADR 0001's no-daemon decision; WPDevPilot's pty is in-process.
...
what WPDevPilot already shipped (tabs, lazy tree, search filter, new
...
1. **"Open in New Tab"** context item — trivial; WPDevPilot's editor already
   has tabs.
...
3. **Virtualized rows** — Superset renders through a virtualized tree;
   WPDevPilot renders all expanded rows. Only matters for huge `wp-content`
   trees; defer until it's felt.
4. **Tree keyboard navigation** (arrow keys expand/collapse/move).
5. Cosmetic: scroll-fade edges, full-pane drop overlay (WPDevPilot's drag-drop
