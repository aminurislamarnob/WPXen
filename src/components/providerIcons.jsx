...
export function ClaudeIcon({ size = 16, className = '', ...props }) {
  return (
    <svg
...
export function CommandCodeIcon({ size = 16, className = '', ...props }) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width={size}
...
export function AntigravityIcon({ size = 16, className = '', ...props }) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width={size}
...
export function MimoIcon({ size = 16, className = '', ...props }) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width={size}
      ...
      {...props}
    >
      <path d="M5 18V11" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" />
      <path
        d="M5 5L12 12L19 5V18"
        stroke="currentColor"
        strokeWidth="2.2"
        strokeLinecap="round"
        strokeLinejoin="round"
...
export function CodexIcon({ size = 16, className = '', ...props }) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width={size}
...
// Brand colors, listed only where the provider actually has one. Most of these
// marks are monochrome *by design* — verified rather than assumed:
//   codex        OpenAI's mark is black-on-light / white-on-dark, no accent
...
// Renders a provider's mark, falling back to a generic terminal glyph so a
// registry entry added without an icon still renders.
//
// `brand` opts into the brand color. Pass it only for available providers:
// the marks are currentColor, so an unbranded one inherits the row's text
// color and dims correctly when a provider isn't installed. Hardcoding the
// fill would make an uninstalled agent look active.
export function ProviderIcon({
  ...
  const Icon = PROVIDER_ICONS[agentId] || Terminal;
  const color = brand ? PROVIDER_BRAND[agentId] : undefined;
  return (
    <Icon
      size={size}
      className={className}
