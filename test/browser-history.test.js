...
describe('upsertEntry', () => {
  it('prepends a new visit', () => {
    const next = upsertEntry([], { url: 'https://wpdevpilot.test' }, 5);
    expect(next).toHaveLength(1);
    expect(next[0]).toMatchObject({
      url: 'https://wpdevpilot.test',
      visits: 1,
      visitedAt: 5,
    });
  });
  ...
  it('dedupes by URL, moving the entry to the front and counting the visit', () => {
    const before = [
      entry('https://a.test'),
      entry('https://wpdevpilot.test', { visits: 3 }),
    ];
    const next = upsertEntry(before, { url: 'https://wpdevpilot.test' }, 9);
  ...
  it('keeps a title and favicon a previous visit learned', () => {
    const before = [
      entry('https://wpdevpilot.test', { title: 'My Blog', faviconUrl: 'https://f.ico' }),
    ];
    // did-stop-loading fires before the favicon arrives, so this visit has
    // neither — it must not wipe what's already known.
    const next = upsertEntry(before, { url: 'https://wpdevpilot.test' });
  ...
  it('lets a new visit update the title and favicon', () => {
    const before = [entry('https://wpdevpilot.test', { title: 'Old' })];
...
describe('searchEntries', () => {
  const entries = [
    entry('https://wpdevpilot.test/wp-admin', { title: 'Dashboard', visitedAt: 300 }),
    entry('https://wpdevpilot.test', { title: 'My Blog', visitedAt: 200 }),
    entry('http://phpmyadmin.test/index.php?route=/&db=wpdevpilot', {
      title: 'phpMyAdmin',
      visitedAt: 100,
    }),
  ];
  ...
  it('ranks a host prefix above a match buried in the URL', () => {
    // "wpdevpilot" is a prefix of two hosts and also appears in phpMyAdmin's query
    // string — the hosts must win.
    const found = searchEntries(entries, 'wpdevpilot');
    expect(found[0].url).toBe('https://wpdevpilot.test/wp-admin');
    expect(found[2].url).toBe('http://phpmyadmin.test/index.php?route=/&db=wpdevpilot');
  });
  ...
  it('breaks ties by recency', () => {
    const found = searchEntries(entries, 'wpdevpilot.test');
    expect(found.map((e) => e.visitedAt)).toEqual([300, 200]);
  });

  it('is case-insensitive', () => {
    expect(searchEntries(entries, 'WPDEVPILOT.TEST')).toHaveLength(2);
  });
...
describe('store wrappers', () => {
  ...
  it('records, searches and clears', () => {
    ...
    history.record(store, { url: 'https://wpdevpilot.test', title: 'My Blog' });
    history.record(store, { url: 'https://other.test' });

    expect(history.search(store, 'wpdevpilot')).toHaveLength(1);
    history.clear(store);
  ...
  it('survives a store holding something that is not a list', () => {
    ...
    expect(history.search(store, '')).toEqual([]);
    expect(() => history.record(store, { url: 'https://wpdevpilot.test' })).not.toThrow();
