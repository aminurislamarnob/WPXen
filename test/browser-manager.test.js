...
describe('navigation', () => {
  it('sanitizes before loading', () => {
    ...
    expect(browser.navigate('browser:1', 'wpxen.test')).toBe(true);
    expect(guest.loadURL).toHaveBeenCalledWith('https://wpxen.test');
  });

  it('reports failure rather than throwing when the tab is gone', () => {
    expect(browser.navigate('browser:gone', 'wpxen.test')).toBe(false);
...
describe('scheme guard', () => {
  ...
  it('lets ordinary pages through', () => {
    ...
    for (const event of ['will-navigate', 'will-redirect']) {
      const e = { preventDefault: vi.fn() };
      guest.emit(event, e, 'https://wpxen.test/wp-admin');
...
describe('context menu', () => {
  ...
  it('offers page actions when the click was not on a link', () => {
    ...
    expect(writeText).toHaveBeenCalledWith('http://wpxen.test/');
